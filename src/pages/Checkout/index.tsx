...
const Checkout = () => {
  ...
  const form = useFormik({
    ...
    onSubmit: (values) => {
      purchase({
        ...
        payment: {
          installements: values.installments,
          card: {
            active: payWithCard,
  ...
  useEffect(() => {
    const calculateInstallmens = () => {
      const installementsArray: Installment[] = []
      for (let i = 1; i <= 6; i++) {
        installementsArray.push({
          quantity: i,
          amount: totalPrice / i,
          formattedAmount: parseToBrl(totalPrice / i)
        })
      }
      return installementsArray
    }
    if (totalPrice > 0) {
      setInstallments(calculateInstallmens)
    }
  }, [totalPrice])
      ...
      {isSuccess && data ? (
        ...
        <form onSubmit={form.handleSubmit}>
          ...
          <Button
            ...
            type="submit"
            title="Clique aqui para finaizar a compra"
            disabled={isLoading}
          >
